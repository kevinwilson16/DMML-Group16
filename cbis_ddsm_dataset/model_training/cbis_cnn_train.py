...
DATA_DIR = Path(__file__).resolve().parent / "cbis_ddsm_preprocessed"
TRAIN_DIR = str(DATA_DIR / "train")
VAL_DIR = str(DATA_DIR / "test")  # acts as validation / test set
...
def build_model(device: torch.device):
    """Build the ResNet-50 model with Dropout head, loss, optimizer, and scheduler."""
    num_classes = 2

    # ResNet-50 with ImageNet weights, full fine-tuning
...
def train_model(
    ...
    best_val_acc = 0.0
    best_model_path = models_dir / "cbis_cnn_resnet50_best.pth"
...
def evaluate_best_model(
    ...
    """Quick evaluation of the saved best ResNet-50 model on the validation set."""
